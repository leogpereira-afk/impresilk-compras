...
function navRede(ativo) {
  return '<nav class="rede-nav" aria-label="Rede de suprimentos">' + [['fornecedores','🏢','Fornecedores'],['transportadoras','🚚','Transportadoras'],['materiais','📦','Produtos e materiais']].map(([r,i,t]) => '<a class="btn' + (r === ativo ? ' primario' : '') + '" href="#/' + r + '"' + (r === ativo ? ' aria-current="page"' : '') + '>' + i + ' ' + t + '</a>').join('') + '</nav>';
}
function opcoesRede(col) { return redeAtivos(col).map(x => ({v:x.id,t:x.nome})); }
let sequenciaRotuloRede=0;
function ligarRotulosRede(el){el.querySelectorAll('.campo').forEach(c=>{const label=c.querySelector('label'),input=c.querySelector('input:not([type="checkbox"]),select,textarea');if(label&&input){if(!input.id)input.id='campo-rede-'+(++sequenciaRotuloRede);label.htmlFor=input.id;}});}
...
function editarMaterial(id, preenchimento={}) {
  const m=achar('mat',id)||preenchimento;
...
TELAS.materiais=function(el,args){
  ...
  cabecalho(m?m.nome:'Produtos e materiais',m?'Uma especificação, os nomes de cada fornecedor.':'Modelos prontos, suas variações e os nomes usados pelos fornecedores.',m?'<a class="btn" href="#/materiais">← Materiais</a><button class="btn" id="editarMat">Editar padrão</button><button class="btn primario" id="novaOferta">+ Vincular fornecedor</button>':'<button class="btn primario" id="novoMat">+ Novo material</button>');
  ...
  const ms=lista('mat').sort((a,b)=>String(a.nome).localeCompare(String(b.nome),'pt-BR')),hist=historicoMateriais();
  el.innerHTML=navRede('materiais')+'<div class="rede-pesquisa">'+campo('Buscar modelo ou produto','<input id="buscaRede" type="search" placeholder="Ex.: ACM poliéster, adesivo, PS, branco…">')+'</div>'+htmlBibliotecaProdutos()+'<section class="cartao"><div class="rede-titulo"><div><span class="rede-kicker">Seu catálogo · '+ms.length+' variações</span><h2>Produtos cadastrados</h2></div><span class="legenda">Ordem A–Z</span></div><div class="produto-salvos">'+ms.map(x=>'<a class="produto-salvo" data-rede-texto="'+esc([x.nome,x.especificacao,x.categoria].join(' '))+'" href="#/materiais/'+esc(x.id)+'"><span class="produto-simbolo" aria-hidden="true">📦</span><div><b>'+esc(x.nome)+'</b><small>'+esc(x.categoria||'Material')+' · '+esc(x.unidade)+' · '+redeAtivos('oferta').filter(o=>o.materialId===x.id).length+' fornecedor(es)'+(x.ativo===false?' · Arquivado':'')+'</small></div><span aria-hidden="true">→</span></a>').join('')+'</div>'+(!ms.length?'<p class="legenda">Escolha um modelo acima para salvar a primeira variação. Depois, vincule os fornecedores na ficha do produto.</p>':'')+'</section>'+
    (hist.length?'<details class="cartao produto-historico"><summary>↶ Materiais que já apareceram nas compras · '+hist.length+' descrições</summary><p class="legenda">Descrições originais para consulta. Nomes parecidos permanecem separados até você confirmar a especificação.</p>'+hist.map((x,i)=>'<div class="produto-salvo" data-rede-texto="'+esc(x.nome)+'"><div><b>'+esc(x.nome)+'</b><small>'+esc(x.unidade)+' · <a href="#/'+x.rota+'/'+esc(x.id)+'">'+esc(x.codigo||'Ver origem')+'</a></small></div><button class="btn pequeno" data-historico-produto="'+i+'">Usar descrição</button></div>').join('')+'</details>':'')+'<p id="redeSemBusca" class="aviso atencao" hidden>Nenhum modelo ou produto encontrado. Você também pode criar um material livre.</p>';
  document.getElementById('novoMat').onclick=()=>editarMaterial(null);ligarBibliotecaProdutos(el);ligarBuscaRede(el);el.querySelectorAll('[data-historico-produto]').forEach(b=>b.onclick=()=>{const h=hist[Number(b.dataset.historicoProduto)];editarMaterial(null,{nome:h.nome,unidade:h.unidade});});
};
